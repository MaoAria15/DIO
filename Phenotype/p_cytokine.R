...
Data_cyto <- subset(Data, !(Group %in% c("FVT_PyT", "FVT_SDT")))
...
count_number <- function(df)
{
  ...
  vector[2] <-  count(df$Group == "Unmodified_FVT")[[2,2]]
  vector[3] <-  count(df$Group == "Obese_control")[[2,2]]
  vector[4] <-  count(df$Group == "Lean_control")[[2,2]]
  return(vector)
}
...
stat_single_compare <- function(df){
  stat_cytokine<- df %>%
    wilcox_test(Parameter~Group,
                comparisons = cytokine_Obse_compare,
                p.adjust.method = "fdr",
                paired = FALSE, 
                alternative = "two.sided",
                detailed = TRUE) 
  stat_cytokine
  stat_cytokine_neg<- df %>%
    wilcox_test(Parameter~Group,
  ...
  stat.test <- stat_cytokine
  stat.test[3,] <-stat_cytokine_neg
  return(stat.test)
}
#plot function
...
ggplot_boxplot_abundance <- function(df)
{
  ggplot(df, aes(x= Group, y= Parameter))+ 
    scale_fill_manual(values = cytokine_cols) +
    ...
    scale_x_discrete(breaks=cytokine_groups,
...
variable <- "GM_CSF"
df<-Data_cyto[,c("Mouse_ID","Group",variable)]
...
p_cytokine
###################################################################################
write_xlsx(sheet, "Phenotype/stat_result/cytokines.xlsx")
