...
set.seed(19950915)
ps.arrival <- subset_samples(PSV.no.Realm.CSS, Sample_time_point %in% "Arrival")
ps.afterfvt <- subset_samples(PSV.no.Realm.CSS, Sample_time_point %in% "1w_after_FVT")
ps.termination <- subset_samples(PSV.no.Realm.CSS, Sample_time_point %in% "Termination")
...
ps@sam_data$Treatment <- factor(ps@sam_data$Treatment, levels = groups)
p_termination <- phyloseq::plot_ordination(ps, GP.ord, axes = 1:3,color="Treatment") + 
  stat_ellipse(geom = "polygon", level = 0.95, fill = NA, linewidth = 1,show.legend = FALSE) +
  geom_point(alpha=.8, size=2,aes(color=Treatment)) +
  scale_color_manual(values= cols)+
...
#                 common.legend = TRUE,
#                 legend = "bottom")
p_beta_vir <- ggarrange(p_arrival,
                             p_afterfvt,
                             p_termination,
                             # p_arrival_stat,
                             # p_afterfvt_stat,
...
# 
 write_xlsx(sheets, "Virome_2/stat_result/bray_permanova_all.xlsx")
# 
 sheet2 <- list(
...
# 
 write_xlsx(sheet2, "Virome_2/stat_result/bray_permanova_pairs.xlsx")
