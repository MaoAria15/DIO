...
ps.rarefied = rarefy_even_depth(PSV.no.realm, rngseed=1, sample.size=2000, replace=F)
...
"Termination_median" = stat_termination_median,"Termination" = stat_termination, "Termination_neg" = stat_termination_neg
  ...
  write_xlsx(sheets, "Virome_2/stat_result/virome_alpha.xlsx")
...
p_afterfvt <- ggplot(tab_afterfvt, aes(x= Treatment, y= Shannon)) +
  ...
  stat_pvalue_manual(stat.test,label = "p.adj", tip.length = 0, size = 3,
                     y.position = c(NA,NA,NA,NA,5.7))+
  theme_classic() +
  mytheme_alpha
p_afterfvt
###############################################################################
...
p_alpha_vir <-ggarrange(p_arrival,
