...
ps <- subset_samples(PSV.HOST, Sample_time_point %in% "Termination")
...
path_table <- "stat_result/desep2_host/"
...
tab_CHP_all <- read.table("stat_result/desep2_host/FVT_ChP_HFD.tsv", sep = "\t",header = T, row.names = 1)
tab_SDT_all <- read.table("stat_result/desep2_host/FVT_SDT_HFD.tsv", sep = "\t", header = T, row.names = 1)
tab_PYT_all <- read.table("stat_result/desep2_host/FVT_PyT_HFD.tsv", sep = "\t",header = T, row.names = 1)
tab_FVT_all <- read.table("stat_result/desep2_host/Unmodified_FVT_HFD.tsv", sep = "\t", header = T, row.names = 1)
tab_LFD_all <- read.table("stat_result/desep2_host/Lean_control_HFD.tsv", sep = "\t",header = T, row.names = 1)
...
ha_row <- HeatmapAnnotation(ObesevsChP=anno_simple(-log10(tax_heatmap$CHP_p),col = pvalue_col_fun, pch = na_if(tax_heatmap$CHP,"ns"), gp = gpar(fontsize(1))),
                            ...
                            which = "row")
rownames(tax_heatmap)<- paste(rownames(tax_heatmap), tax_heatmap$Genus,sep = "_")
rownames(mat_scaled)<- paste(rownames(mat_scaled), mat_scaled$Genus,sep = "_")

ha_row_txt <- rowAnnotation(labels = anno_text(rownames(tax_heatmap),which = "row",gp=gpar(fontsize=4, face= "italic")))
...
plot <- mat_scaled[rownames(tax_heatmap),]
p_host_Deseq_heatmap_simple <- ComplexHeatmap::pheatmap(plot, 
                                                       cluster_cols = FALSE, cluster_rows = TRUE,
                                                       name="Z-score", col=colorRamp2(c(-2, 0, 2), c("dodgerblue4", "white","deeppink3")),
                                                       top_annotation = ha_col,
                                                       left_annotation = ha_row,
