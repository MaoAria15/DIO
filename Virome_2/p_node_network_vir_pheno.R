...
clinical_data <- clinical_data[,(colnames(clinical_data) %in% c( "ewat_mg", "Weight_gain_perc","Cumulative_score","AUC1_w18"))]
...
common_row_names <- intersect(rownames(vir_otu), rownames(clinical_data))
# Keep only rows with common row names in all three dataframes
vir_otu <- subset(vir_otu, rownames(vir_otu) %in% common_row_names)
...
vir_clinical <- cor_tab(vir_otu,clinical_data)


#merge correlationdata together
cor.data <- rbind(vir_clinical)
write.csv(cor.data,"Virome_2//stat_result/phenotypic_vir_spearman_corr.csv")
...
p_bac_pheno_node <- ggraph(g, layout = 'linear', circular=TRUE) +
