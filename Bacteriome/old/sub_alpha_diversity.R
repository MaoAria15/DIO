...
kt_beforefvt <- kruskal.test(data = tab_beforefvt,Shannon~Liver_response)
...
# Get the sample size 
n1 <-  count(tab_beforefvt$Liver_response == "Unmodified_FVT_Non_Response")[[2,2]]
n2 <-  count(tab_beforefvt$Liver_response == "Unmodified_FVT_Response")[[2,2]]
n3 <-  count(tab_beforefvt$Liver_response == "Obese_control_Non_Response")[[2,2]]
n4 <-  count(tab_beforefvt$Liver_response == "Obese_control_Response")[[2,2]]
n5 <-  count(tab_arrival$Liver_response == "Lean_control_Non_Response")[[2,2]]
...
tab_beforefvt$Liver_response <- factor(tab_beforefvt$Liver_response,levels = liver_group)
p_beforefvt <- ggplot(tab_beforefvt, aes(x= Liver_response, y= Shannon)) +
  stat_boxplot(geom ='errorbar', linetype=1, width=0.2) +
  geom_boxplot(outlier.shape = NA,alpha = 1,aes(fill=Liver_response), coef=1.5) +
  geom_jitter(show.legend=FALSE, width=0.25, shape=21, fill="black") +
  stat_summary(fun=mean, show.legend=FALSE, geom="crossbar", linetype=3,
